...
MCResult Ising<N>::mc_sweep(double beta) {
    double norm1 = 1.0 / (MC_SWEEP_COUNT * N * N);
    double exps[4];
    precompute_exp(beta, exps);
    long energy_acc = 0;
    // This tends to overflow (square) - we are performing
    // normalization on the go!
    double energy_sqr_acc = 0;
    long magnetisation_acc = 0;
    long long magnetisation_sqr_acc = 0;
    ...
    for (int swp = 0; swp < MC_SWEEP_COUNT; swp++) {
        ...
        energy_acc += energy;
        energy_sqr_acc += ((energy) * (energy)) * norm1;
    ...
    return {
        (double) energy_acc / (MC_SWEEP_COUNT * N * N),
        (double) magnetisation_acc / (MC_SWEEP_COUNT * N * N),
        beta * ((double) magnetisation_sqr_acc - (double) magnetisation_acc * (double) magnetisation_acc / MC_SWEEP_COUNT) * norm1,
        beta * beta * (energy_sqr_acc - (double) energy_acc * (double) energy_acc * norm1 / MC_SWEEP_COUNT),
    };
}
